      ...
      <div className='container flex flex-row pt-2 justify-between items-center mx-auto'>
        ...
        <div className='flex flex-row gap-2'>
          {menuOptions.map(option => (
            <Link className='capitalize font-gray-800 px-6 py-3 rounded-full hover:bg-[#00056c] hover:text-white last:border-2 last:border-black' href={option.href}>{option.title}</Link>
          ))}
        </div>
      </div>
    </div>
